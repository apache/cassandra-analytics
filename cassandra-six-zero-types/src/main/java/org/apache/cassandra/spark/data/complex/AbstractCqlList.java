...
/**
 * A copy of the four-zero class. Cassandra 5.0 removed {@code UUIDGen.getTimeUUIDBytes()}, so the inherited
 * four-zero class throws {@code NoSuchMethodError} for every list cell that carries a time to live;
 * {@link TimeUUID.Generator} plus {@code TimeUUID.toBytes()} returns the same 16 bytes.
 */
