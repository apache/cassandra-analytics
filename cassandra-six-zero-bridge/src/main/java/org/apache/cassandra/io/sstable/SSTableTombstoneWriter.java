...
public final class SSTableTombstoneWriter implements Closeable
{
    ...
    static
    {
        ...
        // Partitioner is not set in client mode. Cassandra 6.0 makes setPartitionerUnsafe touch
        // StorageService.instance, so take the setter that only assigns the partitioner
        if (DatabaseDescriptor.getPartitioner() == null)
        {
            DatabaseDescriptor.setOnlyPartitionerUnsafe(Murmur3Partitioner.instance);
        }
        // build() commits schema, which reads ClusterMetadata.current(). The call is idempotent, so it costs
        // nothing when CassandraTypesImplementation.setup has already run
    ...
    // The 5.0 constructor also took a ClusteringComparator, used only to build the slices of a range
    // tombstone, which Cassandra 6.0 builds through DeleteStatement.createSlices instead
    ...
    private void rawAddRow(List<ByteBuffer> values) throws InvalidRequestException, IOException
    {
        ...
        List<ByteBuffer> keys = delete.buildPartitionKeyNames(options, state);
        ...
        // Cassandra 6.0 removes UpdateParameters; RegularRowUpdateBuilder is what 6.0's own CQLSSTableWriter
        // uses. It derives the updated columns itself, and takes a null rather than an empty prefetched-row map.
        RowUpdateBuilder params =
                new RowUpdateBuilder.RegularRowUpdateBuilder(delete.metadata,
                                                            state,
                                                            options,
                                                            delete.getTimestamp(TimeUnit.MILLISECONDS.toMicros(now), options),
                                                            options.getNowInSec(TimeUnit.MILLISECONDS.toSeconds(now)),
                                                            delete.getTimeToLive(options),
                                                            null);

        if (delete.hasSlices())
        {
            // Write out range tombstones. Cassandra 6.0 removes StatementRestrictions.getClusteringColumnsBounds.
            Slices slices = delete.createSlices(options);
        ...
        SortedSet<Clustering<?>> clusterings = delete.createClustering(options, state);
        try
        {
            for (ByteBuffer key : keys)
            {
    ...
    public static class Builder
    {
        ...
        public SSTableTombstoneWriter build()
        {
            ...
            TableMetadata tableMetadata = CassandraSchema.apply(schema -> {
                // Cassandra 6.0 commits a schema change through Transactional Cluster Metadata: submit
                // replaces transform. SchemaUpdater.submit also opens the keyspaces; see its javadoc.
                if (schema.getKeyspaceMetadata(SchemaConstants.SYSTEM_KEYSPACE_NAME) == null)
                {
                    SchemaUpdater.submit(schema, SchemaTransformations.addKeyspace(SystemKeyspace.metadata(), false));
                }
                ...
                if (schema.getKeyspaceMetadata(keyspaceName) == null)
                {
                    KeyspaceMetadata ksm = KeyspaceMetadata.create(keyspaceName,
                                                                   ...
                                                                   Types.none(),
                                                                   UserFunctions.none());
                    SchemaUpdater.submit(schema, SchemaTransformations.addKeyspace(ksm, false));
                }
                ...
                if (table == null)
                {
                    ...
                    // Cassandra 6.0 adds compatibleWith(ClusterMetadata) to SchemaTransformation, so it is no
                    // longer a functional interface. The check below copies SchemaTransformations' own.
                    SchemaUpdater.submit(schema, new SchemaTransformation()
                    {
                        @Override
                        public Keyspaces apply(ClusterMetadata metadata)
                        {
                            return metadata.schema
                                   .getKeyspaces()
                                   .withAddedOrUpdated(ksm.withSwapped(ksm.tables.with(finalTable)).withSwapped(types));
                        }
            ...
            return new SSTableTombstoneWriter(writer, preparedDelete, preparedDelete.getBindVariables());
        }
        ...
        private TableMetadata createTable(Types types)
        {
            ...
            // Cassandra 6.0 adds a UserFunctions argument; 5.0 passed no functions
            TableMetadata.Builder builder = statement.builder(types, UserFunctions.none());
            if (partitioner != null)
            {
