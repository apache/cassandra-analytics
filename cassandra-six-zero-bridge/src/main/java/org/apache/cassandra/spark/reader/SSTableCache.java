 ...
 *
 * <p>Cassandra 6.0 needs its own copy of this class, because a {@link CompressionMetadata} of 6.0 holds a
 * reference to a compression dictionary and must release it when this class stops holding the metadata.
 */
...
public class SSTableCache
{
    ...
    private final Cache<SSTable, Optional<CompressionMetadata>>       compressionMetadata = buildCompressionMetadataCache(
    propOrDefault("sbr.cache.compressionInfo.maxEntries", 128),
    propOrDefault("sbr.cache.compressionInfo.expireAfterMins", 15));

    static int propOrDefault(String name, int defaultValue)
    {
    ...
    /**
     * Cassandra 6.0 gives a {@link CompressionMetadata} a reference to a compression dictionary, which owns native
     * zstd tables. Release that reference here, so the tables go away once every reader of the dictionary has also
     * released its own reference.
     */
    private Cache<SSTable, Optional<CompressionMetadata>> buildCompressionMetadataCache(int size, int expireAfterMins)
    {
        return CacheBuilder.newBuilder()
                           .expireAfterAccess(expireAfterMins, TimeUnit.MINUTES)
                           .maximumSize(size)
    ...
    public CompressionMetadata compressionMetadata(@NotNull SSTable ssTable, boolean hasMaxCompressedLength, double crcCheckChance) throws IOException
    {
        ...
        // An operator who turns the memoisation off, or who caps the CompressionInfo size, gets an instance that
        // nobody holds and so nobody closes. Ref's reaper releases the dictionary reference of such an instance
        // after garbage collection, and logs LEAK DETECTED for each one
