        ...
        qt().forAll(arbitrary().enumValues(Partitioner.class))
            .checkAssert(partitioner -> {
                try (TemporaryDirectory directory = new TemporaryDirectory())
                {
                    ...
                    new org.apache.cassandra.io.util.File(String.format("./%s/%s", schema.keyspace, schema.table), dataFile.getFileName().toString()));
                    ...
                    try (InputStream dis = new BufferedInputStream(Files.newInputStream(dataFile));
                         ...
                         RebufferingChannelInputStream in = new RebufferingChannelInputStream(new DataInputStream(
