...
import org.apache.cassandra.cdc.msg.FiveZeroCdcEventBuilder;
...
{
    final Queue<FiveZeroPartitionUpdateWrapper> updates;
    ...
    protected FiveZeroCdcEventBuilder rangeDeletionBuilder;
    private final Random random;
    private final CdcState endState;
    private final CassandraSource cassandraSource;
    private final Double samplingRate;

    protected CdcSortedStreamScanner(@NotNull Collection<FiveZeroPartitionUpdateWrapper> updates,
                                     @NotNull CdcState endState)
    {
        this(updates, endState, ThreadLocalRandom.current(), CassandraSource.DEFAULT, 0.0);
    }

    public CdcSortedStreamScanner(@NotNull Collection<FiveZeroPartitionUpdateWrapper> updates,
                                  @NotNull CdcState endState,
    ...
    {
        return FiveZeroCdcEventBuilder.build(CdcEvent.Kind.ROW_DELETE, partition, row, trackingId, cassandraSource);
    }

    public CdcEvent buildUpdate(Row row, UnfilteredRowIterator partition, boolean isStaticOnly, String trackingId)
    {
        FiveZeroCdcEventBuilder builder = FiveZeroCdcEventBuilder.of(CdcEvent.Kind.UPDATE, partition, trackingId, cassandraSource);
        if (!isStaticOnly)
        {
            builder.withRow(row);
        }
    ...
    {
        FiveZeroCdcEventBuilder builder = FiveZeroCdcEventBuilder.of(CdcEvent.Kind.INSERT, partition, trackingId, cassandraSource);
        if (!isStaticOnly)
        {
            builder.withRow(row);
        }
    ...
    {
        return FiveZeroCdcEventBuilder.of(CdcEvent.Kind.PARTITION_DELETE, partition, trackingId, cassandraSource)
                                      .build();
    }
    ...
    public void handleRangeTombstone(RangeTombstoneMarker marker, UnfilteredRowIterator partition, String trackingId)
    {
        if (rangeDeletionBuilder == null)
        {
            rangeDeletionBuilder = FiveZeroCdcEventBuilder.of(CdcEvent.Kind.RANGE_DELETE, partition, trackingId, cassandraSource);
        }
