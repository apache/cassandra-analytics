...
/**
 * Destructive variants of the multi-DC consistency bulk-read tests: each test either stops instances or
 * redefines Spark-side class bytecode via ByteBuddy. Runs on {@link DestructiveTopologySparkIntegrationTestBase}
 * so every {@code @Test} gets a fresh cluster and sidecar.
 *
 * <p>The non-destructive {@code happyPathTest} remains in {@link BulkReaderMultiDCConsistencyTest}.
 */
public class BulkReaderMultiDCConsistencyDestructiveTest extends DestructiveTopologySparkIntegrationTestBase
{
     ...
     */
    @Test
    void eachQuorumIsNotQuorum() throws NoSuchMethodException, IOException
    {
        ...
        try
        {
            new ByteBuddy()
                ...
                .intercept(
                MethodCall.invoke(BulkReaderMultiDCConsistencyDestructiveTest.class.getMethod("getAvailability", CassandraInstance.class))
                          .withAllArguments()
                    )
                .make()
                .load(
        ...
        {
            // Restore original bytecode so other tests in this JVM (which reuse the CassandraDataLayer class)
            // are not affected by the redefinition, even though each test gets a fresh cluster.
