...
/**
 * Integration tests for SSTable version-based bridge selection in bulk writer, with feature DISABLED
 */
class BulkWriterSSTableVersionBridgeDisabledTest extends SharedClusterSparkIntegrationTestBase
{
    ...
    protected void initializeSchemaForTest()
    {
        ...
        cluster.schemaChangeIgnoringStoppedInstances(
            "CREATE TABLE " + TABLE_DISABLED + " (id int PRIMARY KEY, value text) WITH compression = {'enabled': false};"
        );
    }

    @Test
    void testBulkWriteWithSSTableVersionBasedBridgeDisabled()
    {
        SparkSession spark = getOrCreateSparkSession();

        // Create test data
        List<Row> data = Arrays.asList(
            RowFactory.create(0, "x"),
            RowFactory.create(1, "y"),
            RowFactory.create(2, "z")
        );
        ...
        // Write with feature disabled (fallback to cassandra.version)
        bulkWriterDataFrameWriter(df, TABLE_DISABLED).save();

        // Verify data was written
        SimpleQueryResult result = cluster.coordinator(1)
                                          .executeWithResult("SELECT id, value FROM " + TABLE_DISABLED,
